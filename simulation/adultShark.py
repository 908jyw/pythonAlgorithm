...
def sharkMove():
    ...
    for i in range(M,0,-1):
        ...
        for j in range(4):
            ...
            # if(j==3 and visited_copy[ny][nx][1] != i and ):
            elif (j == 3 and visited_copy[ny][nx][1] != 0):
                for k in range(4):
                    nd = prior_d[i][d - 1][k]
                    ny = cy + dy[nd]
                    nx = cx + dx[nd]
                    # if (i == 1): print('33', ny, nx)
                    if (ny < 0 or ny >= N or nx < 0 or nx >= N):
                        continue
            ...
            elif (visited_copy[ny][nx][0] != 0):
                # if(i == 1) : print('11',ny,nx)
                continue

            # 이동을 한곳에 나보다 큰 애가 있으면 그 큰 상어는 먹힌다 여기서 갈곳의 상태가 흔적이 없는 상태여야지만 가
            elif (area[ny][nx] != 0 and area[ny][nx] > i and visited_copy[ny][nx][0] == 0):
                # if (i == 1): print('22',ny, nx)
            ...
            elif(j != 3 and visited_copy[ny][nx][0] != 0):
                continue
